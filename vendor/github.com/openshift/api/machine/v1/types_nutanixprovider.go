...
type NutanixVMDiskDeviceProperties struct {
	...
	// deviceIndex is the index of the disk address. The valid values is 0 or a positive integer, with the default value 0.
	// For a Machine VM, the deviceIndex for the disks with the same deviceType/adapterType combination should start from 0
	// and increase consecutively afterwards.
